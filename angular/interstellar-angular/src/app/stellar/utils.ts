...
const isValidNewBalance2 = (assetType: string, currentBalance: number, outlay: number): boolean => {
    ...
    const newBalance = (currentBalance - outlay);
    if (newBalance >= 0) {
        if (assetType === 'native') { return (newBalance > StellarLumensMinimum); } else if (assetType === 'tycoin') { return (newBalance > TyCoinMinimum); }
        return true;
    }
    return false;
};
...
const updateBalance = (balanceArray: Array<AccountBalance>, assetType: string, amount: string): void => {
    ...
    const newbal = parseInt(balanceArray[index].balance) - parseInt(amount);
...
export { isValidSecretKey, currencyAssetsMapper, getBalanceforAsset,
        updateBalance, isValidNewBalance, isValidNewBalance2,
        StellarLumensMinimum
};
